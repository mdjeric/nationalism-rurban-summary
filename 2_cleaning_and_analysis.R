...
# Recoding religion
GSS_14_MISS$religion <- rec.relig12(GSS_14_MISS$relig, GSS_14_MISS$denom, GSS_14_MISS$other)
...
clm_small <- lm(criteria_plus ~ age + sex + race + immigrant + veteran + prt_ba + r_degree + 
                  r_coninc + r_srcbelt, data = GSS_14)
clm_small_res16 <- update(clm_small, ~ . + r_res16_c)
clm_small_plvws <- update(clm_small, ~ . + polviews_m)
clm_small_regns <- update(clm_small, ~ . + r_region_4)
clm_small_relgs <- update(clm_small, ~ . + r_religion_m)
clm_full <- update(clm_small, ~ . + r_religion_m + r_region_4 +
                     polviews_m + r_res16_c)